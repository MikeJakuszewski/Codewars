...
function match(candidate, job) {
  const candidatesKeys = Object.keys(candidate);
  const jobsKeys = Object.keys(job);

  if (!candidate[candidatesKeys] || !job[jobsKeys]) throw new Error();

  return (
    candidate[candidatesKeys] <= job[jobsKeys] + candidate[candidatesKeys] * 0.1
  );
}
